...
Процедура ОбработатьСобытиеСообщитьОтправителюОРегистрацииВходящегоДокумента(Знач Событие, Знач ОбработчикПредставление) Экспорт
	...
	Если ТипЗнч(ШаблонПодписи) = Тип("СправочникСсылка.ШаблоныТекстов")
		И ЗначениеЗаполнено(ШаблонПодписи) Тогда
		
		Подпись = Справочники.ШаблоныТекстов.ПолучитьТекстШаблона(ШаблонПодписи);
		
	КонецЕсли;
		
	// TODO: использовать шаблоны текстов
	// В идеальном варианте следует установить получить настройку в хранилище общих настроек
	// для текущего пользователя (администратора) - шаблон ответа и его использовать
	// И если его нет - создать его и установить настройку.
	ШаблонАвтоответаБезИсходящего =	
		НСтр("ru = 'Добрый день!
			|Ваше письмо зарегистрировано %1 под № %2.'");
	ШаблонАвтоответаСИсходящим =	
		НСтр("ru = 'Добрый день!
			|Ваше письмо (исх. от %3 № %4) зарегистрировано %1 под № %2.'");
	ШаблонАвтоответа = ?(ЗначениеЗаполнено(РеквизитыДокумента.ИсходящаяДата), ШаблонАвтоответаСИсходящим, ШаблонАвтоответаБезИсходящего);
	
	ТекстАвтоответа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонАвтоответа,
