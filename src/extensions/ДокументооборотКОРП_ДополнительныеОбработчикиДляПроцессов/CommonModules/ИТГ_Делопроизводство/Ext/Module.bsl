...
Функция ЗначенияРеквизитовДокумента(Знач Документ, Знач Реквизиты) Экспорт
	...
	Если СтруктураРеквизитов.Свойство("Адресат") Тогда
		// TODO: проверять только для внутренних документов!
		...
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно получение адресата документа ""%1"". Для документов вида ""%2"" не используется учёт по адресатам'"),
				Строка(Документ), Строка(РеквизитыДокумента.ВидДокумента)),
			Контекст);
		ОбщегоНазначенияКлиентСервер.Проверить(
			ЗначениеЗаполнено(РеквизитыДокумента.Адресат),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно получение адресата документа ""%1"". В документе не указан адресат'"),
				Строка(Документ)),
			Контекст);
	КонецЕсли;
	...
	Если СтруктураРеквизитов.Свойство("Организация") Тогда
		...
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно получение организации документа ""%1"". В системе не используется учёт по организациям'"),
				Строка(Документ)),
			Контекст);
		// TODO: проверять только для внутренних документов!
		...
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно получение организации документа ""%1"". В документе не указана организация'"),
				Строка(Документ)),
			Контекст);
	КонецЕсли;
	...
	Если СтруктураРеквизитов.Свойство("Подразделение") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Невозможно получение подразделения документа ""%1"". В документе не указано подразделение'"),
				Строка(Документ)),
			Контекст);
	КонецЕсли;
