...
Функция ПолучитьПолнуюРольСОбъектамиАдресацииПоДокументу(Знач Роль, Знач Документ) 
	...
	ВышестоящиеПодразделения = ОбщегоНазначенияДокументооборот.ВсеРодителиЭлемента(РеквизитыДокумента.Подразделение);
	...
	Для Каждого Подразделение Из ВышестоящиеПодразделения Цикл
		...
		//		Подразделение);
		//Иначе
			ПолнаяРоль = Справочники.ПолныеРоли.НайтиСоздатьПолнуюРоль(Роль, Подразделение);
		//КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Вычисление полной роли невозможно. Не указаны исполнители роли ""%2"" для подразделения в документе ""%1""'"),
