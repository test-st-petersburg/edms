...
Функция ИТГ_ПолучитьСписокДоступныхФункций(ИменаПредметовДляФункций)
		...
		Если ИменаПредметовДляФункций.Количество() > 0 Тогда
			...
			Для Каждого ИмяПредмета Из ИменаПредметовДляФункций Цикл 
				...
				ДоступныеФункции.Добавить("ИТГ_ШаблоныБизнесПроцессов.РегистраторПерепискиОтправителя(Объект, ИмяПредмета)",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.Регистратор переписки отправителя'"), Строка(ИмяПредмета)));
	...
	ДоступныеФункции.Добавить("ИТГ_ШаблоныБизнесПроцессов.ВсеСотрудникиМетрологическихПодразделений(Объект)",
		НСтр("ru = 'Все сотрудники метрологических подразделений'"));
