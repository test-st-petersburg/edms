...
// Возвращает значение указанного поля заголовка
//
Функция ПолучитьПолеЗаголовкаИзПараметровОтправкиПисьма(Знач ПараметрыОтправки, Знач ИмяПоля) Экспорт
	
	Для Каждого Поле Из ПараметрыОтправки.ПоляЗаголовка Цикл
		Если Поле.ИмяПоля = ИмяПоля Тогда
			Возврат Поле.ЗначениеПоля;
		КонецЕсли;
	КонецЦикла;
