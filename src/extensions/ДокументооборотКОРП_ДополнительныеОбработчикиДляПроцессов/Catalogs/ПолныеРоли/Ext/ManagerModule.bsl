...
//
Функция НайтиСоздатьПолнуюРольПоРеквизитамОбъекта(Знач ИсходнаяРоль,
	...
	Контекст = "Справочники.ПолныеРоли.НайтиСоздатьПолнуюРольПоРеквизитамОбъекта";
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Поиск исполнителей роли для вышестоящих подразделений возможен только с проверкой наличия исполнителей роли ""%1""'"),
			Строка(ИсходнаяРоль)),
		Контекст);
	ОбщегоНазначенияКлиентСервер.Проверить(
		ИсходнаяРоль.ИспользуетсяБезОбъектовАдресации Или ЗначениеЗаполнено(ОбъектыАдресации),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Роль ""%1"" не используется без объектов адресации. Объекты адресации должны быть указаны'"),
			Строка(ИсходнаяРоль)),
		Контекст);
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru = 'Указана необходимость поиска исполнителей ролей для вышестоящих подразделений, однако в объектах адресации не указано подразделение'"),
		Контекст);
		
	ПолнаяРоль = НайтиСоздатьПолнуюРольПоОбъектамАдресации(ИсходнаяРоль, ОбъектыАдресации);
	...
	ИначеЕсли ИсходнаяРоль.ИспользуетсяСОбъектамиАдресации И ИскатьИсполнителейРолиДляВышестоящихПодразделений Тогда
		...
		Для Каждого Подразделение Из ВышестоящиеПодразделения Цикл
			...
			ПолнаяРоль = НайтиСоздатьПолнуюРольПоОбъектамАдресации(ИсходнаяРоль, НовыеОбъектыАдресации);
			Если БизнесПроцессыИЗадачиСервер.ЕстьИсполнителиРоли(ПолнаяРоль) Тогда
				Возврат ПолнаяРоль;
			КонецЕсли;
		КонецЦикла;
		...
		ОбщегоНазначенияКлиентСервер.Проверить(
			Ложь,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вычисление полной роли невозможно. Не указаны исполнители роли ""%1"" для подразделения ""%2"" и вышестоящих подразделений'"),
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Вычисление полной роли невозможно. Не указаны исполнители роли ""%1"" (полная роль ""%2"")'"),
...
//  основании исходной роли.
//
Функция НайтиСоздатьПолнуюРольПоОбъектамАдресации(Знач ИсходнаяРоль, Знач ОбъектыАдресации)
